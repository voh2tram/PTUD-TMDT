...
function guiForm() {
    var formSubmitted = false; // Biến để theo dõi trạng thái form đã được submit hay chưa

    document.getElementById('myForm').addEventListener('submit', function (event) {
        event.preventDefault(); // Ngăn chặn hành động mặc định của form (tải lại trang)

        if (!formSubmitted) { // Kiểm tra xem form đã được submit chưa
            alert('Tin nhắn đã được gửi thành công');
            formSubmitted = true; // Đặt trạng thái đã submit form
        }
    });
}